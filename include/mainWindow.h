...
Q_OBJECT
  ...
  void showAddSubDialogImpl(std::map<proIds::Uuid, projectDetails> details){
      ...
      connect(addUi.ParentDropdown, &QComboBox::currentIndexChanged, [&addUi, &details](int index){proIds::Uuid parent = proIds::Uuid(addUi.ParentDropdown->currentData().toString().toStdString()); auto pdetails = details[parent]; float perc = (oneMinus(pdetails.assignedSubprojFraction).value/eb_float::fromPercent); addUi.PercentField->setMaximum(perc); addUi.PercentField->setValue(perc/2.0); addUi.PercentHint->setText(displayFloatHalves(perc).c_str());});
